...
const Home = () => {
  const { vendors: data, toggleVendors, isLoading, isError } = useContext(VendorsContext);
  let limit = 7;
    ...
    <div className=" mb-40">
      <div className=" my-3 flex gap-10 items-center justify-center bg-white text-white font-semibold">
        <button
          ...
          disabled={isLoading}
          className=" p-2 bg-cyan-200 hover:scale-105 active:scale-95 transition-all"
        >
      ...
      {isLoading ? (
        ...
        <Vendors data={data.slice((currentPage - 1) * limit, currentPage * limit)} />
      )}
      <Pagination
        totalPage={totalPage}
        limit={limit}
